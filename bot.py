...
from fun import cmd_spam, cmd_fuck
...
async def handle_deleted_event(event: BusinessMessagesDeleted):
    ...
    for msg_id in event.message_ids:
        if cid in cache and msg_id in cache[cid]:
            ...
            fallback = (
                f'<tg-emoji emoji-id="5366573795404447728">🗑</tg-emoji> '
                f'Сообщение было удалено, но не сохранено в кеше.\n'
                f'ID сообщения: <code>{msg_id}</code>'
            )
            try:
                await bot.send_message(owner_id, fallback, parse_mode="HTML")
            except Exception as e:
                logging.error(f"Ошибка fallback-уведомления об удалении: {e}")
