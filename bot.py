...
from fun import cmd_spam, cmd_fuck, cmd_stata
from db import init_db, load_all_stats, save_user_stats
...
# chat_id -> user_id -> статистика (загружается из SQLite при старте)
user_stats: dict[int, dict[int, dict]] = {}


def update_chat_stats(message: Message):
    ...
    if user_id not in user_stats[chat_id]:
        user_stats[chat_id][user_id] = {
            "messages":    0,
            "stickers":    0,
            "gifs":        0,
            "video_notes": 0,
    ...
    if message.date < s["first_date"]:
        s["first_date"] = message.date
    if message.date > s["last_date"]:
        s["last_date"] = message.date

    s["username"] = message.from_user.username or ""
    s["name"] = message.from_user.full_name

    save_user_stats(chat_id, user_id, s)
...
async def handle_business_message(message: Message):
    ...
    update_chat_stats(message)

    sender_id = message.from_user.id if message.from_user else None
    cmd = get_command(message.text or "")

    if cmd in ("spam", "fuck", "stata"):
        ...
        if cmd == "spam":
            ...
            await cmd_stata(message, bot, user_stats)
        return
...
START_TEXT = (
    ...
    '<b>После подключения \nбот будет:</b>\n'
    '<blockquote>'
    '🐸Пересылать Удаленные сообщения.\n'
    '📷Пересылать удаленные фото/видео.\n'
    '🎤Пересылать удаленные Голосовые сообщения и аудио.\n'
    '🎭Пересылать удаленные GIF и Стикеры.\n'
    '🎥Пересылать удаленные кружочки.'
    '</blockquote>\n\n'
    '<b>Пользуйтесь!</b> 🤖'
)
...
async def main():
    init_db()
    global user_stats
    user_stats = load_all_stats()
    logging.info(f"Загружена статистика для {sum(len(v) for v in user_stats.values())} пользователей")
